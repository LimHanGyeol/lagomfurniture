...
public class PageMakerUtils {
    ...
    private int contentnum;// 한 페이지에 보일 게시물의 개수, 리뷰 기준 10 / 상품 기준 6
    private int startPage = 1;// 현재 페이지 블록의 "시작" 페이지
    private int endPage;// 현재 페이지 블록의 "마지막" 페이지, 리뷰기준 10 / 상품기준 5
    ...
    public void prevnext(int pagenum, int pageLimit) {
        // 현재 페이지가 첫번째 페이지 블록 안에 있으면,
        // 이전 페이지로 가는 화살표는 보이지 않는다. 다음페이지 화살표는 보인다.
        // pageLimit = 10페이지 기준 11, 5페이지 기준 6
    ...
    // setPage = 10페이지 기준 10 / 5페이지 기준 5 입력
    public void setStartPage(int currentblock, int setPage) {
        // 10 페이지로 할거면 10 - 9 로 바꿔주기
        this.startPage = (currentblock * setPage) - (setPage - 1);
    }
    ...
    public void setEndPage(int getlastblock, int getcurrentblock, int setPage) {
        if (getlastblock == getcurrentblock) {
            ...
            // setPage = 10페이지 기준 10 - 1 = 9 / 5페이지 기준 5 - 1 = 4
            this.endPage = getStartPage() + (setPage - 1);
        }
    ...
    public void setCurrentblock(int pagenum, int blockNum) {
        // 페이지 번호를 통해서 구한다.
        // 현재 페이지 번호가 1 페이지 라면
        ...
        // blockNum = 10페이지 기준 10 / 5페이지 기준 5
        this.currentblock = pagenum / blockNum; // 1페이지당 10블록
    ...
    public void setLastblock(int totalcount, int blockNum) {
        // 10, 5 => 10*5 => 50
        // 125 / 50 = 2.5 . 나머지가 0보다 크기 때문에 1을 더하면 3.
        // 페이지 블럭이 3까지 존재 한다.
        // blockNum = 10 페이지 기준 10 / 5페이지 기준 5
    ...
    public void setLastPage() {
        this.lastPage = calcpage(getTotalcount(), getContentnum());
    }
